...
screen_science.py
=================
Screens 10 Science keyword candidates from Dimensions.ai (1975-2024).
...
Results are saved incrementally to screen_science_report.csv.

Run from the directory containing Dim_key.txt:

    python screen_science.py
...
# ---------------------------------------------------------------------------
# 10 Science candidates
# (chosen to span physics, chemistry, earth science, biology, astronomy)
# ---------------------------------------------------------------------------
CANDIDATES = [
    ("black_holes",          ["black holes"]),
    ("CRISPR",               ["CRISPR"]),
    ("quantum_computing",    ["quantum computing"]),
    ("climate_change_atmos", ["atmospheric climate change"]),
    ("protein_folding",      ["protein folding"]),
    ("exoplanets",           ["exoplanets"]),
    ("graphene",             ["graphene"]),
    ("CERN_particle",        ["particle physics collider"]),
    ("stem_cells",           ["stem cells"]),
    ("superconductivity",    ["superconductivity"]),
]

REPORT_CSV = Path("screen_science_report.csv")
...
def fetch_corpus(name, keywords):
    """Fetch corpus from Dimensions and return a processed DataFrame."""
    all_records = {}
    for kw in keywords:
...
def compute_stats(name, df):
    """Compute network richness statistics from a corpus DataFrame."""
    n_papers = len(df)
    if n_papers == 0:
        return None
...
for name, keywords in CANDIDATES:
    ...
    ir_pct  = f"{stats['internal_ratio']:.1%}"
    gcc_pct = f"{stats['gcc_fraction']:.1%}"
    print(f"\n  RESULT: n={stats['n_papers']:,}  "
          f"internal_ratio={ir_pct}  gcc_fraction={gcc_pct}  "
...
report_sorted = report.sort_values("gcc_fraction", ascending=False)

print("\n\n" + "="*70)
print("SCIENCE CANDIDATES — SUMMARY (sorted by gcc_fraction)")
print("="*70)
cols = ["dataset","n_papers","mean_ref_len","n_internal_edges",
        "internal_ratio","gcc_fraction","n_components","verdict"]
cols = [c for c in cols if c in report_sorted.columns]
print(report_sorted[cols].to_string(index=False))
